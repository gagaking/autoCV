...
bbox 格式：[ymin, xmin, ymax, xmax]。其中 ymin, xmin, ymax, xmax 均是 0 到 100 之间的百分比实数，数值代表该瑕疵区域在图二（生成海报图）中的百分比平面位置。必须严格遵守[纵向起始, 横向起始, 纵向结束, 横向结束]的顺序！
例如：[30.0, 20.5, 60.0, 45.5] 表示一个从纵向 30.0% 到 60.0%（y轴），并且横向从 20.5% 到 45.5%（x轴）的矩形框。⚠️请确保由于模型底层机制，务必优先输出y坐标！
  ...
  "issues": [
    {
      ...
      "desc": "问题描述内容",
      "bbox": [ymin, xmin, ymax, xmax] // 0-100 的实数百分比坐标, 顺序: ymin, xmin, ymax, xmax
    }
  ]
}
`;
